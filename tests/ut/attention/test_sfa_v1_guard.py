...
def test_model_runner_tracks_actual_request_count_separately_from_padding():
    ...
    assert "def _build_lightning_indexer_metadata(" in source
    ...
    assert "prefix_num_tokens = int(li_cum_query_lens[num_actual_reqs - 1]) if num_actual_reqs > 0 else 0" in source
    assert "all requests are fully skipped by lightning_indexer_skip" in source
    assert "self.lightning_indexer_metadata = self._build_lightning_indexer_metadata(" in source
    assert "prev_lightning_indexer_metadata = self.lightning_indexer_metadata" in source

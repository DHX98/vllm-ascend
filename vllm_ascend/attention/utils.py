...
def get_sfa_skip_indices(
    num_computed_tokens: np.ndarray,
    query_lens: np.ndarray,
    skip_threshold: int = 2048,
):
    # calculate num of tokens need skip for each request
    num_computed_tokens = num_computed_tokens[: len(query_lens)]
    num_skip_tokens = np.maximum(skip_threshold - num_computed_tokens, 0)
    ...
    li_seq_lens = np.concatenate(
        [
            ...
            skip_query_lens[skip_query_lens != 0] + num_computed_tokens[skip_query_lens != 0],
        ]
    )
...
def maybe_pad_and_reorder_inputs(input_ids, positions, reorder_indices):
    ...
    positions_reorder_pad = torch.zeros_like(positions)

...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.def(
        ...
        "Tensor key, Tensor value, Tensor beta, Tensor initial_state, "
        "Tensor actual_seq_lengths, Tensor? g=None, "
        "float scale_value=1.0) -> (Tensor out, Tensor final_state)");
    ops.impl("npu_chunk_gated_delta_rule", torch::kPrivateUse1,
             &vllm_ascend::npu_chunk_gated_delta_rule);
